<div align="center">

# 📬 Resumen de Visitas

<sub>PORTAFOLIO 2026 · REPORTE AUTOMÁTICO | README ACTUALIZADO EL · %{{updated_at}}</sub>

</div>

---

<div align="center">

### Visitantes Totales

# %{{visitors}}

</div>

---
<div align="center">

| # | Métrica | Valor | Cantidad |
|:---:|:---|---:|---:|
| 📍 | **Ciudad más frecuente** | %{{first_city}} | %{{first_city_count}} |
| 📍 | **Segunda ciudad** | %{{second_city}} | %{{second_city_count}} |
| 📍 | **Tercera ciudad** | %{{third_city}} | %{{third_city_count}} |
| 📍 | **Cuarta ciudad** | %{{fourth_city}} | %{{fourth_city_count}} |
| 📍 | **Quinta ciudad** | %{{fifth_city}} | %{{fifth_city_count}} |
| 🌍 | **País más frecuente** | %{{first_country}} | %{{first_country_count}} |
| 🌍 | **Segundo país** | %{{second_country}} | %{{second_country_count}} |
| 🌍 | **Tercer país** | %{{third_country}} | %{{third_country_count}} |
| 🌍 | **Cuarto país** | %{{fourth_country}} | %{{fourth_country_count}} |
| 🌍 | **Quinto país** | %{{fifth_country}} | %{{fifth_country_count}} |
| 🖥️ | **Sistema Operativo habitual** | %{{os}} | |

`Última visita desde %{{city}}, %{{country}} el %{{created_at}} • %{{time_ago}}`

</div>
---

<div align="center">

<sub>© %{{year}} · @solidsnk86</sub>

</div>